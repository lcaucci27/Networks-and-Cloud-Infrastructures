...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _request_ports_stats(self, datapath):
        self.logger.info('Requesting port stats from Switch ID: %016x', datapath.id)
    ...
    def calculate_stats(self, switch_id, port_no, new_tx_bytes, new_rx_bytes):
        ...
        time_elapsed = self.get_time_elapsed(switch_id=switch_id, current_time=current_time, port_no=port_no)
    ...
    def get_time_elapsed(self, switch_id, port_no, current_time):
        previous_time = self.switch_times[switch_id].get(port_no)
        ...
        if switch_id not in self.switch_ports:
            ...
            for id, switch_info in self.switch_ports.items():
                ...
                if switch_info.get('type') == arp.ARP_REQUEST:
                    ...
                    self._initialize_blocked_ports(id, port)
    ...
    def _initialize_blocked_ports(self, switch_id, port):
        if switch_id not in self.blocked_ports:
            self.blocked_ports[switch_id] = {}
        if port not in self.blocked_ports[switch_id]:
            self.blocked_ports[switch_id][port] = False
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            if arp_packet.opcode == arp.ARP_REQUEST and eth.dst == 'ff:ff:ff:ff:ff:ff':
                if arp_packet.src_ip not in self.host_ports:
                    ...
                    self.logger.info("ARP broadcast request from %s (MAC: %s) on Switch DPID %s, Port %s",
                                    arp_packet.src_ip, arp_packet.src_mac, datapath.id, in_port)

                    with open("host_ports.txt", 'a') as file:
                        file.write(f"IP: {arp_packet.src_ip} collegato a Switch: {datapath.id} su porta: {in_port}\n")

                    self.switch_ports[datapath.id] = {
                        'ip': arp_packet.src_ip,
                        'mac': arp_packet.src_mac,
                        'port': in_port,
                        'type': arp.ARP_REQUEST
                    }

                    with open("switch_ports.txt", 'a') as file:
                        file.write(f"Switch: [{datapath.id}]\n Porta: [{in_port}]\n Host-IP: [{arp_packet.src_ip}]\n Tipo: REQUEST\n\n")
                ...
                if arp_packet.src_ip not in self.host_ports:
                    ...
                    self.logger.info("ARP reply from %s (MAC: %s) on Switch DPID %s, Port %s",
                                    arp_packet.src_ip, arp_packet.src_mac, datapath.id, in_port)
                    ...
                    if (self.host_ports[arp_packet.src_ip]['mac'] != arp_packet.src_mac or
                        ...
                        self.logger.info("Host information updated for %s", arp_packet.src_ip)
